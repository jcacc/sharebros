...
class itysl(commands.Cog):
    ...
    @commands.command(name='itysl', help='itysl flow')
    async def itysl(self, ctx, word: str = None):
        try:
            with open('/home/jca/dev/python/sharebro/cogs/itysl.txt', 'r') as file:
            ...
            await ctx.send('The quotes file could not be found. Make sure "itysl.txt" is in the correct directory.')

# Required function to add this cog to the bot (Synchronous Version)
async def setup(bot):
    await bot.add_cog(itysl(bot))
